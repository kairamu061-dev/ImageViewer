...
| ホイールスクロールを wheelEvent 上書きで1ノッチ=1/5行に | 既定では wheelScrollLines×大セルでほぼ1ページ飛んでしまうため。1行でもまだ大きいとのフィードバックで1/5行に。スクロールバーの singleStep も updateGeometries で同値に再設定（QListView が上書きするため） |
